...
const newArrivalsProducts = allProducts.filter(p => p.isNew);
...
const NewArrivals = () => {
  ...
  const [products, setProducts] = useState(newArrivalsProducts.map(p => ({ ...p, image: '' })));
  ...
  useEffect(() => {
    ...
    async function fetchImages() {
      ...
      const prods = await Promise.all(
        newArrivalsProducts.map(async (p) => {
    ...
    <section className="py-24 bg-stone-50 animate-fade-in">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-serif font-light text-stone-900 mb-6">New Arrivals</h2>
